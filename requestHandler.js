            ...
            ffmpeg.stderr.on('data', function (data) {
                ...
                for(var i = 0;i<lines.length;i++) {
                  if(lines[i].match(/Duration/)) {
                    ...
                    //finalTime = rawFinalTime.split(':').join()
                    //console.log('here is duration ' + finalTime);
                  }
                 //console.log('this is array: ' + i + '  ' + lines[i]);
                    ...
                    if(!(time.length < 1)) {
                      ...
                      console.log(timeUp);
                      //res.write(timeUp[0] + ' ');
                      //var bytesUploaded = timeUp[0];
                      //var bytesTotal = finalTime;
                      //var pc = parseInt(100 - (timeUp[0] / finalTime * 100));
                   // res.write(timeUp[0]+'<br />');
//res.write(pc);
                      //console.log(timeUp[0]);
    ...
    if(code == 0) {
      do {
        ...
        client.on('connect', function() {
          ...
          client.expire(downloadKey, 
                        settings.settings.redisCacheTime, 
                        redis.print);
