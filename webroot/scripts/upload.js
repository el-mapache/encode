...
function prevDef(e) {
    console.log('im supposed to stop shit');
    e.stopPropagation();
    e.preventDefault();
}

//prevents the browser from attempting to play the audio file(in theory)
...
function getMetrics(file) {
    ...
    $('#fileName').html('<b>Name: </b>' +file.name);
    $('#fileSize').html('<b>Size: </b>' + fileSize);
    $('#fileType').html('<b>Type: </b>' + file.type);
    if(file.lastModifiedDate) {
        var lastMod = file.lastModifiedDate.toLocaleDateString();
        $('#lastMod').html('<b>Last Modified: </b>' + lastMod);
    }
}
